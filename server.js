...
// Helper function to generate SVG badge with improved styling and proper text spacing
function generateSVGBadge(contributors, style = 'horizontal', theme = 'light') {
  const avatarSize = 32;
  const padding = 8;
  const usernameHeight = 16;
  const spacing = 4;
  const minItemWidth = 60; // Minimum width for each contributor item
      ...
      contributorElements += `
        <g class="contributor" style="cursor: pointer;">
          <a href="${contributor.html_url}" target="_blank">
            ...
            <text x="${x + avatarSize/2}" y="${y + avatarSize + spacing + 12}" text-anchor="middle" font-family="Arial, sans-serif" font-size="10" fill="${colors.text}" class="username">${displayName}</text>
          </a>
        </g>
      `;
    });
